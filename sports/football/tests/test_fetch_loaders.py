...
right ``nflreadpy`` function with the right keyword args, converts the (polars) result to
pandas, and renames ``stats_player`` columns back to the pipeline's canonical schema. A fake
``nflreadpy`` module is injected so nothing touches the network or the real library.
"""
...
class _FakeFrame:
    """Stand-in for a polars DataFrame: the loaders only use ``.to_pandas()``."""
...
def _install(monkeypatch, frame: pd.DataFrame) -> list[dict]:
...
def test_player_stats_loaders_summary_level_and_renames(monkeypatch):
    ...
    assert isinstance(out, pd.DataFrame)  # polars -> pandas conversion happened
...
def test_rosters_renames_gsis_id_to_player_id(monkeypatch):
    _install(monkeypatch, pd.DataFrame({"gsis_id": ["00-1"], "age": [25]}))
...
def test_yearless_loader_called_without_seasons(monkeypatch):
    ...
    calls = _install(monkeypatch, pd.DataFrame({"gsis_id": ["x"]}))
...
def test_rename_skips_columns_not_present(monkeypatch):
    ...
    _install(monkeypatch, pd.DataFrame({"player_id": ["x"], "team": ["KC"]}))
    out = REGISTRY["seasonal"].loader([2024])
